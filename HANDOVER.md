...
**Last updated:** 2026-07-31 (Session 40: **срез #3 — вход ОТМЕНЁН, критерий (a) провален 0.1386 при пороге 0.15 (проход #2 = календарный артефакт). Починено 5 вещей: база hodl (врала +50 вместо −16), BUG-025, критерий (b) → `EDGE vs PARKED`, BUG-024, дашборд. НЕ ЗАДЕПЛОЕНО**).

## 🚨 ПЕРВОЕ ДЕЙСТВИЕ СЛЕДУЮЩЕЙ СЕССИИ — ДЕПЛОЙ

Все правки Session 40 лежат **только локально**. На сервере крутится старый
код. Нужно:
```bash
pnpm deploy:hetzner          # + пересоздать контейнер (compose start НЕ читает env)
```
После деплоя проверить: (1) `netΔ` в логах должен стать ≈ −0.003, а не
−0.07 (BUG-025); (2) в `/opt/delta-bot/data/watchdog.log` в течение 5 минут
появится строка `VITALS-EPISODE-EXPIRED: … (бот перезапущен)` и
`vitals_open` в `watchdog.state` опустеет (BUG-024). Файл
`data/hodl-baseline.json` надо положить на сервер тоже — иначе дневной крон
среза продолжит писать фальшивый +50.
