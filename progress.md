...
### Session 45 — поиск «≥1 % в день чистыми» по всем стратегиям: НЕ СУЩЕСТВУЕТ устойчиво; наш бот при нынешней скачке SOL ≈ 0 или минус; найдено: сервер Hetzner удалён, 5 USDC ушли 2 Sep (ждут подтверждения оператора)

**Задание оператора:** «проверь, есть ли пулы, где можно заработать сейчас, какой лучший, и найди стратегию хотя бы на 1 % в день», затем «проверь все возможные стратегии… роллапы, новости Твиттера и т.д., сколько угодно агентов».
Четыре workflow (~60 агентов, у каждого направления свой проверяющий, отдельный поиск пропущенного и сводка). Полный отчёт: `docs/reports/2026-09-25-1pct-day-strategy-sweep.md`.

**1. Главный механизм (подтверждён дважды на разных источниках).** Для LP с хеджем решает отношение R = (комиссии пула за день ÷ глубина D на единицу ln-цены) ÷ (σ²/2). И комиссии, и потери от прохода цены через диапазон ∝ 1/W, поэтому ширина R не меняет. R < 1 — минус при любой ширине. Для +1 %/день на весь капитал при σ 3.3 % нужно R ≥ 1.62. Измерено: SOL/USDC старый `5rCf1DM8LjKTw4YqhnoLcngyZYeNnQqztScTogYHAS6` 0.73–1.00, кампания `BGm1tav58oGcsQJehL9WXBFXF7D27vZsKefj4xJKD5Y` 0.71–0.80, шаг 20 `BVRbyLjjfSBcoyiYFuxbgKYnWuiFaF9CSXEa5vdSZ9Hh` 0.97–1.06, Raydium `3ucNos4NbumPLZNWztqGHNFFgkHeRMBQAVemeeomsUxv` 0.77–0.93, Orca `Czfq3xZZDmsdGdUyrNLtRhGc47cXcZtLG4crryfu44zE` 0.40–0.74, cbBTC/USDC `7ubS3GccjhQY99AYNKXjNJqnXjaokEdfdV915xnCb96r` 1.05–1.53 (последние 24ч 0.62–0.78). SOL σ_d 3.3 % (до ралли 1.85–1.9, дисперсия ×3). Июль по факту: комиссии ÷ потери 0.92–0.96.

**2. Наш бот сейчас.** Сим: 678 настроек × окна 7/14/30д — 0 настроек с 1 %/день; честно −0.5 … +0.15 %/день (старый пул). 24 исторических окна с мая: лучшая настройка (20 корзин, выдержка 60) в среднем −0.05 USD/день, в плюс вышли 11 окон из 24; комиссии ÷ потери 1.21 при σ 1.96 → 0.73 при σ 4.25. Потолок при 100 % сборе комиссий, нулевых накладных и любой ширине ≈ +0.16 %/день ≈ +0.7 USD/день. Кампания проходит порог плотности (7д 0.266), но R < 1: **критерий плотности прибыль не предсказывает.**

**3. Весь Solana.** Просмотрено 799 пулов DLMM + 673 DAMM v2, подробно 69 + 22. С хеджем устойчивых ≥1 % нет. Мемы: комиссии 2–40 %/день при падении токенов −39…−67 % за 7д. PumpSwap с момента выхода на биржу: медиана −88 % в SOL. DAMM v2 запуски: медианные комиссии за всю жизнь пула 0.31–0.40 USD. cbBTC/SOL `HDhWhQCBrSh9xNWmNtsTi86eWj3yCoEiaRodjgNydo1b` — единственный с R > 1 все 36 дней, но это 0–1.4 USD/день.

**4. Всплеск с 6–9 Sep — причина найдена он-чейн:** StonkFun (Raydium LaunchLab, конфиг `6BwHHDg3u1854jC8PDLXvR4spTcLNaoBxLJNGC4nTESt`) и список разрешённых активов pump.fun (`6z6GDdfb2AjR9ZhJmAUQ5cipJCVxQvLJhB2H8mCwTFBP`, 93 актива добавлены 9 Sep 19:40–20:34 UTC) — мемкоины, оценённые в акциях/DOGE/wXMR/JTO, прокачивают сделки через пулы этих активов. Почасовые прогоны с хеджем на Hyperliquid (версии проверяющих, вперёд): SPCX `7nVQtYQipN564E9oBi6yZFGox4WL6CR4zsLyYipqgTWu` 0–0.4 %/день (поток рухнул 23 Sep), XMR DAMM v2 `GgB61FHx6PbuCixFnVat2KrDMDA3kCKLjep9rGKUN2Wd` 0.4–0.65 медиана (мост с одним ключом `Ds4prSZNwyxTz4PZmHXoHDXFzLZ1c8MkfhUwGtqvAvpK`), DOGE Raydium `7s9GwyhHTmszPpzYtLaUDFW3LYfuS6XCnFWrGqUk5X4n` ≈0.6 медиана, JTO/SOL Raydium `JVoPtWWDsRcLvQosu5fWc2CaNF6jEtJzbxdPtcEuvZo` ≈0. Один общий источник → одна ставка, не распределение риска.

**5. Все прочие классы (16, у каждого свой проверяющий).** Устойчиво с поправкой на риск ≥1 %/день: **ни одного**. Только эпизодами: Polymarket награды (вперёд 0.1–0.5, закрыто для РФ/DE), отрицательный фандинг (монету в нужный момент не дают занять), бот ликвидаций (только дни обвалов), продажи токенов (2–4 раза в год). Реальный потолок устойчиво 0.01–0.10 %/день. Келли: 1 %/день со сложным процентом требует Sharpe ≥ 2.7 на новых данных — не показала ни одна стратегия.

**6. Попутно — безопасность и состояние.** (a) `hcloud server list` пуст: сервера бота нет (5 Sep, проверка Session 44, он ещё был: оба контейнера Exited, crontab закомментирован — значит удалён между 5 и 24 Sep); `167.233.105.131` отвечает ключом ED25519 `SHA256:IRtgWHZBHknscbZPGGeb37RERacJO5bBCV79UFtoL+A` вместо записанного `SHA256:IIiAZHmL8TaJIWeBfQDRudktclZ+NNWFikuC55uJRXo`; серверный `pnl.db` и логи, вероятно, потеряны (локальный от 28 Jul). (b) 2 Sep: 5 USDC → `5EqfmTh7yMuDeTjTmYaqi7M38ZYvLTNifJn2hQJZMGtL`, подпись ключом кошелька, tx `4dFRNu3Jio4Luiuk68Jt6EvdcqUTgM7CA5uNVDjs8HrgwZqqfuKoP7EtxPMvTq9qk4E2BNTS3menH9EfBQ9tHnrg`. (c) 14/16 Sep — Flash Trade закрылся (16 Sep) и сам вернул остатки. Кошелёк: 2.748145289 SOL + 107.808931 USDC + 0.008488252 HYPE ≈ 427 USD. (a) и (b) оператор подтвердил 25 Sep: сервер удалил он, перевод его (700 USD, о которых он говорит, шли с другого кошелька — здесь только 5 USDC).

**7. Исправления методики** (записаны в BACKLOG A26 и в скилы): вход по R вместо плотности; закон 2 strategy-analyzer опровергнут (выходы из диапазона ∝ 4σ²/W² → потери/день ∝ 1/W); доля LP в Meteora 90 %, DAMM v2 80 %; сим пересоздаёт позицию только на основную сумму — нужен флаг COMPOUND/FIXED_DEPOSIT.

**Продолжение (25 Sep):** оператор попросил исследовать шаблоны торговли мемкоинами + «кошелёк Робингуда» + сигналы Твиттера → запущено как BACKLOG A27 (сборщик данных, гипотезы/протокол, разбор соцсигналов — три фоновых агента).

**Итог мемкоинов (25 Sep, вечер):** сборщик + загрузка 7 дней истории (остановлена: кончился месячный лимит Helius на ≈1 млн единиц); по приказу оператора анализ на ≈2 днях собранного — выживший шаблон C1 (создатель сам покупает 0.05–0.2 SOL) УБИТ: −2.99 %/сделку, выигрыш 5 %, ДИ целиком < 0; 0 из 266 вариантов уверенно в плюсе; публичный набор данных был перекошен в пользу активных монет. Исследование закрыто (BACKLOG A27).

**Рекомендация оператору:** цель «1 %/день» снять; сейчас — JupSOL + Jupiter Lend USDC (≈5 % годовых); бот не запускать; Meteora S2 — минутная проверка на официальной странице до 2026-10-21 10:00 UTC (по цепочке ≈173 USD комиссий в окне ≈5.6 MET < минимума 10).

---

## 2026-08-28

### Session 43 — два исследования по запросу оператора: (1) хранилище Kamino JUP/SOL `CVCsJFoYjN4gxABhuw71buKhGALRbBm5KvVJQcaodVpt` — ОТКЛОНЕНО под мандатом; (2) идея «коррелированная пара без хеджа» — ОТКЛОНЕНА измерениями; попутно: площадка SOL/USDC ожила (старый пул 0.59–0.61 %/день за 7д), но это пока спайк ралли — нужна вторая неделя

**Задание оператора:** «Check this: kamino.com/liquidity/CVCs… worth it?» и затем «рынок ожил — взять пару токенов, которые растут пропорционально и которым не нужен хедж».
Два workflow (18 агентов, 3 «адвоката дьявола», 2 сводки). Все файлы-источники в scratchpad сессии; ключевые цифры ниже.

**1. Kamino JUP/SOL — что это.** Под капотом — **тот же пул Meteora DLMM `C8Gr6AUuq9hEdSYJzoEpNcdjpojPZwqG5MtQbeouNNwg`** (шаг 80 = 0.8 %/бин, комиссия 0.15 %, protocol share 10 % он-чейн — datapi врёт 5 %), который мы отклоняли в Session 34. Позиция `3dPMzVyq9G32YJqN95xjko6wPsuEayhMJ443wrjc2p8t`: 51 бин, равномерная ликвидность, диапазон −16.1 % / +26.0 % от цены; правило `PricePercentageWithReset` 1820/2200 bps, сброс 7500 (срабатывает при −11.7 % / +19.1 %); 1 перестановка за 30 дней (14 Aug 07:11Z), 43 за жизнь. Флаг API `strategyOutOfRange=true / priceLower 1e74` — баг API (бины DLMM парсятся как тики Orca), он-чейн позиция в диапазоне 721/721 ч. Хранилище = 69 % ликвидности пула в диапазоне (54 % TVL) — оно и есть глубина пула. Официальное (isCommunity=0, админ `F7hoimgoK4T3nkjft4Zd3gHThPwXTZjvKiAuUUHdknKX` = Squads-мультисиг Kamino).
**Комиссии Kamino (он-чейн, подтверждены транзакциями):** performance **9.5 %** от собранных комиссий, в натуре при каждом сборе (казна получила ровно 0.0950 за 48 ч), без high-water mark; вывод **0.10 %**; депозит 0. Наград/стимулов нет (rewardMints [USDC] — устаревшие метаданные). **Счётчики комиссий и APR в API уже ЧИСТЫЕ от 9.5 %** (доказано: 0.905 × claimed fees позиции = счётчик стратегии, до 6 знаков).

**2. Доходность — реальная, не заголовочная.** Плотность комиссий на TVL хранилища (чистая): 7 дней 20–26 Aug **0.21–0.25 %/день** (80–91 % годовых); 30 дней **0.084–0.092** (31–34 %); тихое окно 28 Jul–19 Aug **0.040** (13.5–15 %). Поправка проверяющего: тихое окно — **исторический минимум** (0-й перцентиль из 529 скользящих 30-дневных окон с 2025 г.); долгосрочная норма: медиана 0.1136 %/день ≈ **41 % чистыми**, p25 0.087, p75 0.142; ралли-неделя — 93-й перцентиль. Пул целиком (оборот × 0.15 % ÷ TVL): 7д 0.1672 (запас 11 %, проходят 4 дня из 7 → по правилу среза #3 НЕ проход), 30д 0.0607, до 19 Aug никогда выше 0.063.
**Вкладчик (цена доли `/v2/strategies/…/history`):** 30д **+34.1 % в USD** — но это ралли SOL; против удержания своих же токенов **≈0…+2 %** (зависит от часа входа: комиссии +2.6 % ≈ IL от JUP/SOL −16.6 %); 7д +1.4–1.5 % против удержания (соотношение стояло); **за всю жизнь с Feb 2024 −15.5 % против удержания** (JUP/SOL −62 %). Отток: −11 % долей за 30д, два выхода по 190k и 339k долей сразу после рывка 19–21 Aug.
**Модель на 400 USD, 30 дней:** комиссии ≈10 USD/мес при 30-дневной плотности, ≈26 при плотности ралли; ход соотношения ±25 % = IL 17–32 USD; безубыточность для ±25 % 0.16–0.24 %/день — выше 30-дневного среднего; реальный прошедший месяц: **−7…−8 USD против удержания** (IL −18…−19 после одной перестановки, комиссии +11). Перестановка Kamino не мгновенная: после 14 Aug хранилище ~58 ч сидело на 80 % в JUP (swapUneven дросселирован 30 bps от оракула) — в трендовом падении это некомпенсированный риск.
**Хедж:** нога SOL (183 USD из 400) — Jupiter-шорт, carry 5.1–5.6 % = 0.03 USD/день; нога JUP (217 USD) **в машине нехеджируема** (Jupiter Perps: SOL/wETH/wBTC). Снаружи: Flash Trade JUP-short `Hi8kSmbtzucZpEYxvcq2H1QuyUCRuY3m7WGmTF2RhkVw` — займ 45 % годовых, свободно ~188 USDC (наш шорт съел бы всё); Hyperliquid JUP-PERP (шорт сейчас ПОЛУЧАЕТ фандинг ~27 % за 7д, знак меняется) и Bybit/Binance — вручную, другая цепь. Полные 400 в хранилище + хедж + резерв = 493 USD > кошелька; потолок 325, реалистично 200 (и тогда 1.59 SOL простаивает, а USDC не хватает 26).
**Риски протокола:** программа `6LtLpnUFNByNXLyCoK9wA2MykKAmQNZKBdY8s47dehDc` обновляемая, authority = Squads v4 4-из-7 с таймлоком 24 ч, последний редеплой 24 Aug 2026, аудитов новее OtterSec Nov 2023 нет, инцидентов не найдено; Kamino Liquidity TVL 87.9M (−70 % за год); депозиты отбиваются при расхождении с оракулом (error 6059, 9 отказов 27 Aug).
**ВЕРДИКТ: под мандатом (дельта-нейтраль, вход ≥0.15 с запасом) — НЕ КЛАСТЬ.** Это ставка на JUP+SOL с комиссионной надбавкой, а не доходность: за месяц она дала ≈0 против простого удержания тех же монет, а за жизнь — минус.

**3. «Коррелированная пара без хеджа» — измерено, отклонено.** (a) **Стабильного соотношения ни у одной ликвидной пары нет.** Волатильность соотношения за 30д (× SOL/USD 51.7 %): cbBTC/SOL 0.58× (по часовым 0.72×), ETH/SOL 0.67×, RENDER/SOL 0.75×, JUP/SOL 0.95×, HYPE 1.19×, MET 1.56×, PUMP 2.94×. Дрейф соотношения к SOL за 30д: JUP −17.6 %, JTO −32.8, BONK −28.9, RENDER −25.3, BTC −13.4, ETH −9.9, TRUMP +23, PUMP +72. В окне сима 13–27 Aug ни одна из 10 пар не удержалась в ±3 % от SOL. LST-пары (JitoSOL/SOL и т.п.) стабильны (1–2 %), но плотность 0.001–0.002 %/день — в 70 раз ниже порога, комиссии < carry. (b) **«Мало хеджа» неверно в USD-учёте:** для пары X/SOL один SOL-шорт должен быть h = 0.5(β+1) от депозита — JUP 0.96, ETH 0.95, BTC 0.80 против 0.50 у SOL/USDC (шорт в 1.5–1.75× больше); остаточная волатильность после шорта 2–3× нашей (JUP 30 %, ETH 16 %, BTC 11 % против 15–17 %); плюс IL соотношения, которое комиссии не покрывают (JUP: 56 % при d30). **Бот такой шорт считать не умеет** — `computeLpHedgeDelta` получает tokenX как «sol» (pairConfig), на HYPE/SOL хедж поэтому был выключен. В SOL-учёте идея верна, но это = работать без хеджа, что A23 уже проиграл. (c) **Площадки (7д / 30д / до-спайка, %/день):** cbBTC/SOL `HDhWhQCBrSh9xNWmNtsTi86eWj3yCoEiaRodjgNydo1b` 1.05/0.39/0.15 (концентрация ±1 % всего 2.2× — шаг 2); ETH/SOL `3potGhhaNyzvCNq1CGh2wLbZLLMjpdPtG1Hpj8q2DDVR` 0.66/0.24/0.10 (объём 2.48M → 94k за 8.5 ч 28 Aug — затухает); RENDER/SOL `B5dL1cPzmzhz3F4ucoUzKnTNSnpybEoHrC4McTYMHkef` 1.46/0.74/0.45, но TVL 27k; JUP/SOL 0.19/0.068/0.025. Мёртвые: RAY, ORCA, WIF, PYTH, TRUMP, BONK, JTO/SOL (0–12k USD/день). (d) **Сим 13–27 Aug (SOL +37.4 %, band0.49/band99, bins 28):** SOL/USDC старый хедж EvP +0.17 (82.9 % вне пула), кампания +10.96; JUP +2.2/+2.7 (bins 20: −4/−9), ETH +8.5 (bins 20: −4.25 — знак от числа бинов = шум), cbBTC −0.28 (92.5 % вне), JTO −27.7, TRUMP +34 = катание на пампе (190 штормов). Положительные числа на мёртвых пулах (WIF, PYTH, RAY) — фикция класса A24.

**4. Попутная находка — наша площадка ожила, но пока это спайк.** Старый пул `5rCf1DM8LjKTw4YqhnoLcngyZYeNnQqztScTogYHAS6`: **7д (21–27 Aug) 0.585–0.611 %/день, 30д 0.226–0.236, до-спайка (29 Jul–18 Aug) 0.10–0.12** — d30/d7 = 0.39. ⚠️ Первое чтение 0.6756 было завышено на ~20 %: `reserve_in_usd` GeckoTerminal показывал 4.69M при он-чейн TVL 5.6–5.9M — **для TVL брать он-чейн/datapi, не GT.** Кампания `BGm1tav58oGcsQJehL9WXBFXF7D27vZsKefj4xJKD5Y` 0.45/0.16. Оборот старый/кампания 3.75× > гандикапа 2.5× — площадка по-прежнему старый пул. Множитель концентрации ±0.5 %: 18.2× (27 Aug 15:30) → 6.6× (28 Aug 10:50) — снимок гуляет ×2.8 за 19 ч. **Правило входа требует вторую неделю подряд: окно 28 Aug–3 Sep ≥ 0.1725 на он-чейн TVL.** Ожидание при возврате (LP 320.88 / шорт 160.44 / залог 52.94 / резерв 0.3 SOL): грубые комиссии 4.5–14 USD/день при d30 (по множителю 6.6–20×), чистыми в тренде 0.8–7.5, в боковике 3.8–12; carry 0.025 USD/день. Pyth Hermes отдаёт 401 — `jupiter:read` живёт на одном Jupiter-прайсе (починить до входа).

**Итог оператору:** Kamino — нет; коррелированная пара — нет (на цифрах не существует, а хедж выходит больше и хуже, чем у SOL/USDC); единственный кандидат с работающей машиной — старый SOL/USDC при втором проходе плотности. Решение о возврате — за оператором.

---

## 2026-08-17

### Session 42 — SHUTDOWN по приказу оператора: бот остановлен, хедж полностью закрыт, все средства в кошельке

**Задание оператора:** «Stop the bot and withdraw hedging stuff. So all my funds are back to the wallet.»

1. **Сервер:** `docker compose stop delta-neutral-bot` на Hetzner — контейнер остановлен (Exited 0). Hype-контейнер уже стоял с конца июля. Watchdog-cron НЕ отключён (правка crontab заблокирована пермишенами локального агента) — будет слать «bot down» примерно раз в час (`REALERT_SECS=3600`), пока оператор не закомментирует строки `watchdog.sh` в root-crontab. Cron `hodl-compare` тоже жив, но exec в стоящий контейнер просто падает в err-файл.
2. **LP:** позиций не было (парковка со среза #3) — снимать нечего.
3. **Хедж:** SHORT 2.447717646107668 SOL (нотионал 186.25471 USD, коллатерал 61.330601 USDC) закрыт полностью. Dry-run ok, затем LIVE TX1 `2UR7BGEKgqWbLKB61WwmPfiZgHYcnZzeV5qKhen5M8cc8Snpf7aFYSWgpExr36B9ZoKsMCaRpVBUKotcxswP56od` (request `6yJgGx9oKeUhRsKVSNLyWCCcsGJsFCxRatXHfmf1W7aN`), keeper-fill подтверждён: `jupiter:read` → обе стороны null, notional 0, collateral 0.
4. **Итог кошелька:** до 2.747831168 SOL + 49.996952 USDC → после **2.747826168 SOL + 112.808623 USDC** (+62.811671 USDC = коллатерал 61.33 + положительный uPnL расчёта; −0.000005 SOL комиссия TX1). wSOL нет (шорт — USDC-коллатерал). Пыль на Flash Trade (ручной тест 29 Jul) этим не затрагивается.
5. На чейне у кошелька больше нет ни LP, ни перп-позиций. Возобновление: `docker compose start` НЕ перечитывает env — при рестарте пересоздавать контейнер.

---

## 2026-08-01

### Session 41 — срез #4 парковки: стерильное окно (0 транзакций, 0 действий хеджа, 0 VITALS), вход снова закрыт по плотности 0.1441 при пороге 0.15; подтверждено, что деплой Session 40 всё-таки состоялся; найдена документационная дыра (scripts/pool-fee-depth.ts никогда не коммитился)

**Задание оператора:** «сделай срез».

**1. Срез (окно 31 июля 11:01:44Z → 1 августа 15:50Z, 28.81 ч).**
`equity = 2.747841168 × 72.74583286 + 53.115732 + 0 +
(60.816637 + 6.408889732 − 0.140160)` = **320.095093 USD**.
Против среза #3 (320.363802) — **−0.268709**, разложено до цента:

| член | формула | USD |
|---|---|---|
| остаточная дельта | 2.747841168 × (−0.789300) + 1.934551 (uPnL шорта) | −0.234320 |
| carry (borrow) | −(0.140160 − 0.105771) | −0.034389 |
| **сумма** | | **−0.268709** ✓ |

Эффективная остаточная дельта = 0.296870 SOL — это резерв кошелька
0.3 SOL, который ПО ЗАМЫСЛУ не входит во вход хеджа (ADR-021 берёт только
SOL сверх резервов). Хедж работает: netΔ −0.003129729684428817 при полосе
0.25, «no action / in band» все 28.81 ч.

**Бенчмарки:** vs «всё в USDC» (= PnL к базе) **−16.425461**; vs
«ничего не делать» −10.956935, из которых механика защиты
0.812640989 × (79.47515894 − 72.74583286) = **+5.468526**, остаток
умения −16.425409 ≈ строка vs-USDC (расхождение 0.00005) → **утечки
нейтральности нет**; vs «всё в SOL» +12.068430.

**2. Проверка (обязательный блок).** Окно стерильно:
- `scripts/tx-audit.ts --since 2026-07-31T11:01:44Z` → **0 транзакций**,
  комиссий 0 SOL, net ΔSOL 0, net ΔUSDC 0.
- `hedge_actions` за окно: **0 строк** (последняя — 2026-07-27T18:45:05.821Z
  increase_short 184.706809265707 USD). `rebalances` 0 — LP нет.
- Логи: **0 `🚨 VITALS BREACH`**, 0 `[error]` внутри окна. Три `[error]` в
  `bot1.log` — все ДО окна: два одиночных RPC 504 (30 июля 18:38:19Z,
  31 июля 09:17:32Z, оба с `hedgeConsecutiveErrors: 1`) и баннер
  `⚠️ HEDGE IS LIVE` при старте контейнера 10:54:54Z.
- Циклы: итерации 160215 → 164592 = 4 377 за 65 751 с = **15.02 с/цикл**,
  разрывов >60 с нет; рестартов 0.
- Нормы: комиссии сети 0 SOL (норма 0.001–0.005 SOL/сут) ✓; живой оборот
  хеджа 0 при потолке 3× auto-cap ✓; LP нет (припаркованы); ликвидация
  99.913278 = **1.3735× спота** ✓ (порог 1.3); carry 0.0287 USD/сут =
  0.2006 USD/неделю при ставке −559.94 bps на ноционал 184.706809.

**3. Деплой Session 40 БЫЛ сделан — шапка HANDOVER врала.** Контейнер
поднят 2026-07-31T10:54:51Z. Проверено на сервере: `sideBaseSol()`
байт-в-байт совпадает с локальным (BUG-025; живой netΔ −0.0031, а не
−0.07), `deploy/hetzner/watchdog.sh` содержит ветку `(бот перезапущен)` и
она отработала 2026-07-31T10:55:01Z (`VITALS-EPISODE-EXPIRED`,
`vitals_open` пуст — BUG-024 закрыт полевой проверкой),
`data/hodl-baseline.json` на сервере = 336.52055387901737, строка крона от
01 августа 00:17Z считает от неё.

**4. Критерий входа (a) — снова провал.** 7 полных дней Jul 25–31,
GT дневной объём ÷ TVL × базовая комиссия:

| пул | TVL | 7-дневное среднее | было (срез #3) |
|---|---|---|---|
| старый `5rCf1DM8LjKTw4YqhnoLcngyZYeNnQqztScTogYHAS6` | 5 069 395 | **0.1441 %/день** | 0.1386 |
| кампания `BGm1tav58oGcsQJehL9WXBFXF7D27vZsKefj4xJKD5Y` | 2 956 005 | **0.0925 %/день** | 0.0894 |

Старый пул в 4% от порога 0.15 — по правилу, записанному в срезе #3, это
НЕ проход (и не второй проход подряд). Критерий (b) не перемерялся:
(a) — жёсткие ворота, а неделя назад `EDGE vs PARKED` был −2.6906/−2.4569
при пороге +2.

**5. Найдено:** `scripts/pool-fee-depth.ts` цитируется в HANDOVER.md,
BACKLOG.md и progress.md, но **никогда не коммитился**
(`git log --all -- scripts/pool-fee-depth.ts` пуст). Замер плотности
пришлось пересобрать из сырого рецепта GeckoTerminal. Решение за
оператором: закоммитить скрипт или убрать упоминания.

---

## 2026-07-31

### Session 40 — срез #3 парковки: критерий (a) провален (0.1386 при пороге 0.15 — проход #2 оказался календарным артефактом), вход отменён. В кошельке найдены ЧУЖИЕ для бота транзакции на Flash Trade (подтверждено оператором как ручной тест). Починены 4 вещи: восстановлена база hodl, BUG-025 (дельта перпа по споту), критерий (b) симулятора переписан на `EDGE vs PARKED`, BUG-024 (залипший vitals_open), и косметика дашборда

**Задание оператора:** «сделай срез и напиши мне финальный вердикт»;
затем «1 верни, 2 спиши, 3 забей, 4 почини, 5 почини, 6 почини, 7 почини».

**1. Срез (окно 28 июля 00:17Z → 31 июля 12:02Z, 3.49 дня).**
`equity = 2.747841168 × 73.37503006 + 53.115732 + 0 +
(60.816637 + 4.866746 − 0.104611)` = **320.317432 USD**.
Против 28 июля (323.1908891689303) — **−2.8735**, разложено:
Flash Trade −2.5880, цена на остаточную дельту −0.1430, carry −0.0981;
сумма −2.8291, невязка −0.0444 (разное время снимков цены).
Витальные: ликвидация 99.927775 = 1.3627× спота; netΔ −0.0031 при полосе
0.25; carry 0.199 USD/неделю (подтверждено дважды: из ставки 561.954 б.п.
× 184.706809 = 0.0284/день; из накопленного долга 0.104611 / 3.72 дня =
0.0281/день).
Непрерывность доказана арифметикой, а не «на глаз»: в `bot1.log` счётчик
итераций 135068 → 147643 за 188 879 с = 15.02 с/цикл; в `bot.log` 9 818
циклов, медиана 15.0 с, максимальный разрыв 55.1 с, разрывов >60 с ноль.
Рестартов 0, VITALS 0, штормов 0, мутаций хеджа 0. Три одиночные ошибки
RPC, каждая с `hedgeConsecutiveErrors: 1`.

**2. Главная находка — Flash Trade.** 29 июля 14:45–14:58Z, 11 транзакций,
3 подписаны нашим кошельком, программа
`FLASH6Lo6h3iasJKWDs2F8TkW2UKf3s15C8PMGuVfgBn`. В репозитории нет ни
строчки про Flash; в `pnl.db` нет записей; в логах нет строк. Скан 2000
последних подписей кошелька до 22 декабря 2025 — Flash встречается **ровно
один раз**. Внесено 10 USDC, выведено 7.64. **Оператор подтвердил: свой
ручной тест**, 2.36 USDC списаны по его указанию. Важно на будущее: формула
капитала НЕ видит балансы на Flash — при повторном использовании площадки
её надо явно добавлять в срез.

**3. Что починено.**
- **База hodl восстановлена** (`data/hodl-baseline.json`): два carve-out
  теста HYPE возвращены (+0.601069348 и +0.238870613 SOL), totalUsd
  269.766191978485 → **336.52055387901737**. До этого `pnpm hodl` считал
  вернувшиеся ~66.75 USD прибылью и показывал **+50.55** вместо честных
  **−16.17**. Проверка на утечку нейтральности сошлась в ноль: из −11.31
  против «ничего не делать» механика защиты даёт +4.96, остаток −16.20
  совпал со строкой vs-USDC до 0.0000.
- **BUG-025** — оба места (`getHedgeState`, `maybeRebalance`) переведены на
  общий помощник `sideBaseSol()`, делящий на цену ВХОДА. Живьём: шорт
  −2.5189017020711537 → **−2.450970897684429**, подразумеваемая цена входа
  75.36067 против записанной 75.36120572. netΔ −0.0711 → **−0.0031**:
  старое число было на 96% багом. `hedge-vectors.jsonl` перегенерирован
  без изменений — контроллер не тронут, только его вход.
- **Критерий (b) переписан** — в `SimReport` добавлены `parked_end` и
  `edge_vs_parked` (тот же мешок, один шорт на всё окно, только carry).
  `edge_vs_hold` оставлен, но помечен диагностическим в комментарии, в
  выводе CLI и в скиле. Демо-окно показывает патологию одной строкой:
  **+0.4951 против «ничего не делать», но −0.7793 против парковки**.
- **BUG-024** — настоящая причина оказалась не той, что предполагалась
  (предложенный «перестраивать из окна сканирования» вернул бы ложное
  «восстановился» от 10 июля). Реальная: латч живёт в ПРОЦЕССЕ бота, и
  после пересоздания контейнера 26 июля бот физически не мог выдать строку
  восстановления для эпизодов, залипших до старта. Добавлены время открытия
  эпизода и два новых пути закрытия (рестарт новее эпизода; потолок 24ч).
  Проверено на 8 случаях, включая реальное залипшее состояние.
- **Дашборд** — `outOfBand` считался как `|netΔ| > band`, игнорируя цель;
  теперь как у контроллера, `|netΔ − target| > band`, плюс в снимке поля
  `targetDeltaSol`/`errorSol`, а в TUI подпись показывает цель.
- Пропущено по указанию оператора («3 забей»): отзыв `HCLOUD_TOKEN` в
  `deploy/hetzner/host.env` — токен живой, лежит открытым текстом.

**4. Проверка входа — ОТМЕНЁН, критерий (a) провален.** Плотность за 7
полных дней 24–30 июля: старый пул **0.1386 %/день** (было 0.1680),
кампания **0.0894** (было 0.1029) при пороге 0.15. Проход на срезе #2 был
**календарным артефактом**: из окна выпали сильные 21–23 июля и вошли
выходные (25 июля 0.0379, 26 июля 0.0742) — при том что 27 и 29 июля были
отличными (0.2167 и 0.2390). Критерий переформулирован: порог надо
перекрывать С ЗАПАСОМ либо получить два прохода подряд. Выбор площадки не
изменился — оборот старого пула к кампании 3.90× против гандикапа 2.5×.
Новая метрика на том же окне: `EDGE vs PARKED` −2.6906 (старый пул) и
−2.4569 (кампания) — оба критерия против входа.

**Тесты:** 143 vitest + 19 cargo зелёные, `tsc` чистый.
**Не задеплоено:** правки в `deploy/hetzner/watchdog.sh` и коде бота живут
пока только локально — нужен `pnpm deploy:hetzner` и пересоздание
контейнера.
...
### Session 39 — проверка входа #2: ОБА критерия формально пройдены (плотность старого пула 0.168 при пороге 0.15; недельный сим-edge +4.99 при пороге +2), но ВХОД ОТКЛОНЁН — критерий (b) дефектен (база hold-as-is нехеджирована, на падающей неделе дарит +5 за работу хеджа; по абсолюту вход −1.28 против парковки −0.19). Площадка перевернулась на СТАРЫЙ пул. Создан скил `lp-army-playbook` из 14 транскрипций буткемпов Meteora. Опровергнут накопитель волатильности как индикатор живости; измерена нулевая аренда bin array за 179 открытий

**Задание оператора:** сначала — разобрать материалы буткемпов LP Army и
сделать из них скил; затем «запускай» (симулятор) и «тогда паркуемся».

**1. Скил `lp-army-playbook` (новый).**
Путь: `.claude/skills/lp-army-playbook/` — `SKILL.md` (370 строк),
`reference/knowledge-base.md` (10 разделов), `reference/numeric-index.md`
(40 порогов), `reference/disagreements.md` (15 противоречий инструкторов),
`reference/sources.md` (провенанс), `scripts/pool-fee-depth.ts`.
- Источник: 14 из 15 сессий плейлиста Meteora LP Army (≈24ч видео).
  Одна испанская (`nyHSuqW8iGY`, Día 2) недоступна — YouTube не отдаёт ни
  одной дорожки субтитров ни на одном языке/формате; пробел записан, не
  замазан. Публичный `timedtext` теперь отвечает пустым телом, поэтому
  качали `yt-dlp` в изолированном venv.
- Разбор: 15 агентов веером (14 извлекающих + сводящий), 830 фактов,
  ≈1.5 млн токенов, 0 ошибок. Каждый факт помечен силой:
  `consensus` / `single-source` / `disputed`.
- Ключевое проектное решение: буткемпы учат РУЧНОМУ НАПРАВЛЕННОМУ LP на
  мем-коинах, у нас автомат + дельта-нейтраль + голубая фишка. Поэтому
  раздел 1 — наши собственные замеры, и он объявлен старше видео;
  раздел 7 прямо перечисляет непереносимое (все направленные формы —
  bid-ask/«зубная паста»/флип ломают вход хеджа; лестница комиссий для
  мемов; ручной вход по MACD/RSI; проверки на скам).
- Два спора инструкторов закрыты нашим измерением: (1) базовая и
  динамическая комиссия складываются, а показывают итог — обе стороны
  описывали одно с разных концов; (2) потолки динамической комиссии из
  видео к нам не переносятся, переменная часть растёт как КВАДРАТ шага
  бина, а все их числа сняты на шагах 100–250 против наших 4 и 10.
- Ловушка `$<цифра>` в SKILL.md проверена и вычищена (59 попаданий в
  справочных файлах заменены на форму «число USD»).

**2. Проверка входа #2 — полностью в BACKLOG A23 «PARKING WATCH LOG»
Check #2.** Кратко: плотность старого пула 0.1680 %/день (проход),
кампании 0.1029 (провал); площадка перевернулась на старый пул (оборот
4.08× > 2.5× гандикапа ставки, глубина втрое); сим-edge +4.99 (проход),
НО абсолют −1.28 против парковки −0.19 → **вход отклонён**.

**3. Найден дефект критерия (b) — главный результат сессии.**
`simulator/src/strategy.rs:882`: `hold_as_is_end = deposit_sol0 × p_end +
deposit_usdc0` — база НЕХЕДЖИРОВАННАЯ. За окно SOL сходил 78.12 → 73.2
(−6.3%), база потеряла 6.27, и весь EDGE +4.99 — это работа хеджа, а не
доходность LP. Критерий в нынешней формулировке разрешит вход на ЛЮБОЙ
падающей неделе. Предложено переписать на абсолютное Δequity против
парковки — **НЕ применено, ждёт оператора**.

**4. Опровергнуто собственное утверждение (сделано и исправлено внутри
сессии).** В первой редакции скила я записал накопитель волатильности ÷
максимум как дешёвый индикатор живости пула. Проверка на обоих пулах в
один день: живой пул 410/300000 = 0.14%, слабый 10000/350000 = 2.9% —
слабый в 20 раз выше. Период затухания 600 с делает его измерителем
минут, а не потока. Вывод в скиле переписан на «не использовать»,
процедура сравнения переведена на сделки в час и семидневный оборот.

**5. Измерено (не предположено): аренда bin array = НОЛЬ.**
Просканированы все 179 сигнатур открытия позиций из `data/pnl.db`, 0
пропущено. Единственная встречающаяся сумма ренты — `0.057406080 SOL`
(аккаунт позиции, ×176), и закрытие обнуляет тот же аккаунт на ту же
сумму. Невозвратной аренды массивов не платили ни разу ни на одном пуле.
В симулятор эту статью не добавлять.

**6. Множители концентрации (новое, ончейн).** Старый пул ±10 бинов
18.3–23.3×, ±14 бинов 14.3×; кампания ±10 16.5×, ±14 12.0×. Это СНИМОК:
за час чтение по старому пулу сместилось 23.3 → 18.3. Внутри сессии я
ошибочно применил 18.3× (это ±10, то есть 20 корзин) к 28-корзинной
конфигурации — верно 14.3×, валовая оценка 2.40 %/день, не 3.07;
исправлено в ответе и в скиле. Побочная проверка: комиссии LP в симуляторе
упали 13.49 → 10.59 (−21%) при переходе b20 → b28, что совпало с падением
множителя 18.3 → 14.3 (−22%) — две независимые модели сошлись.

**7. Состояние парковки подтверждено, живых операций не было.**
`AUTO_CREATE_POSITIONS=false`, хедж цикл ~15 с, netΔ −0.084 SOL при цели
0, «in band», контейнер 2 суток, 0 ошибок и 0 VITALS. Кошелёк
2.750948478 SOL + 55.475732 USDC. Цена SOL 73.17 сверена независимо
(Jupiter Lite v3 73.167 / Pyth 73.165 / оба пула 73.19).

**8. Безопасность.** В `deploy/hetzner/host.env` открытым текстом лежит
действующий `HCLOUD_TOKEN` с правами на запись — попал в вывод команды.
Файл в `.gitignore`, в репозиторий не утёк, но токен стоит отозвать и
перевыпустить в панели Hetzner (действие оператора).

**Открытые пункты после сессии:** переформулировка критерия (b);
отзыв токена Hetzner; при входе — сменить `METEORA_POOL_ADDRESS` на
старый пул, `AUTO_TUNE_BIN_COUNT` на 28, полосу на 0.49 и пересоздать
контейнер.
...
### Session 38 — срез парковки #1: окно стерильно, TOTAL 324.47 (+0.51 за 21.8ч, объяснено до цента); ОБА критерия входа в SOL/USDC ПРОВАЛЕНЫ (плотность 0.088/0.142 %/день при пороге 0.15; недельный сим −1.77 при пороге +2) → остаёмся в парковке; найден BUG-025 (дельта перпа считается через СПОТ, а не через цену входа)

**Задание оператора:** «Сделай срез и проанализируй. Если надо переедь на
сол усдс и обнови хедж».

**1. Живость и стерильность окна (проверено, не со слов).**
- Контейнер Up, рестартов 0, старт 2026-07-26T11:36:59Z. Циклов с
  рестарта **5235** за 21.84ч (ожидание 5242), максимальный разрыв
  **80.2 с** (один RPC-икот), разрывов >60 с — 1.
- `scripts/tx-audit.ts --since 2026-07-26T11:20:00Z`: **3 транзакции, все
  три — само сворачивание** (закрытие HYPE-LP 11:32:01Z, пылинка от
  keeper 11:32:12Z, продажа HYPE 11:34:15Z); после 11:34Z Jul 26 —
  **ноль движений кошелька**. Комиссии окна 0.000289957 SOL.
- `hedge_actions` с 11:30Z Jul 26: **0 строк** — контроллер не принял ни
  одного решения кроме «в полосе» (пишутся все не-`none`, включая
  сухие).
- VITALS за окно: **0**. В постоянном логе только два загрузочных
  баннера «HEDGE IS LIVE» (11:35:45Z с целью −1.97 — до пересоздания,
  11:37:01Z с целью 0 — после) и три одиночных RPC 504 от 25–26 июля
  (каждый = 1 подряд, самовосстановление). Сторож: `vitals_open` всё ещё
  залипший — **BUG-024**, известен.

**2. Портфель (формула с числами).** Цена SOL 76.22475 (Pyth), Jupiter
76.22424, расхождение 0.0007%.
```
TOTAL = SOL 2.750958478 × 76.22475           = 209.691
      + USDC 50.812475                       =  50.812
      + шорт: залог 66.980435 + uPnL (−2.953381) − borrow 0.063604 = 63.963
      ────────────────────────────────────────────────────────────
                                               = 324.467
```
Против финальной отметки A23 **323.96** → **+0.51 за 21.8 ч**. Разложение
(окно стерильно, значит объяснить обязано всё): остаточная дельта
портфеля 0.4306 SOL × рост цены +1.235 USD = **+0.532**, минус прирост
платы за заём шорта −0.025 → **+0.507**. Совпало до цента.
Ликвидация 103.588654 = **1.359× спота** ✓ (норма ≥1.3; пробьёт 1.3 при
SOL ≈ 79.7, тревога 1.25× — при SOL ≈ 82.9).
Цена парковки: carry −5.46% годовых на $173.91 = **$0.79/месяц**.

**3. Критерий (a) — плотность комиссий: ПРОВАЛЕН.** Порог 0.15%/день.
GT дневные обороты ÷ TVL × базовая ставка:
- пул кампании `BGm1tav58oGcsQJehL9WXBFXF7D27vZsKefj4xJKD5Y` (TVL 2.917M,
  ставка 0.1%): среднее за 7 дней **0.088%/день**; лучший будний день
  (22 июля) 0.148; суббота 25 июля 0.028.
- старый пул `5rCf1DM8LjKTw4YqhnoLcngyZYeNnQqztScTogYHAS6` (TVL 4.991M,
  ставка 0.04%): среднее за 7 дней **0.142%/день**; будни 0.19–0.30,
  выходные 0.038.
- Перевес старого = **1.60×**, что меньше 2.5× разницы ставок → венью не
  меняем (правило A24 в силе).
- **Поправка к рецепту:** оборот вырос в 2.5× за сутки — замер 26 июля
  («пул умер, $135/час») пришёлся на воскресную яму. Плотность мерить
  **средним за 7 дней**, точечное чтение врёт в разы.

**4. Критерий (b) — недельный edge симулятора: ПРОВАЛЕН.** Порог +2 USD.
Каноническая рамка (`--swap-skip --lp-value 148 --idle-sol 0.8
--wallet-usdc 41 --confirm-min 10 --reentry-min 120 --reentry-tol 0.20`),
неделя 168ч до 2026-07-27T09:00Z:
- кампания шаг10/fee6.5: bins 28 band 0.25 **−1.77** (abs −1.79),
  bins 28 band 0.49 −2.01, bins 20 band 0.25 −0.34;
- старый пул шаг4/fee2.5: bins 28 −1.20 (78% времени вне пула),
  bins 70 −3.58.
Другие окна тот же знак: 336ч −0.62, месяц 720ч −9.82 (abs −1.00),
последние 72ч −2.54. Важно: симулятор уже щедр к комиссиям (начисляет
нашим проходам ≈0.45%/день на капитал LP против 0.09% по пулу) — и всё
равно не дотягивает до нуля.

**5. Вердикт: НЕ ВХОДИМ, стоим в парковке.** Оба предварительно
согласованных критерия провалены с запасом, причём независимо друг от
друга. Парковка стоит $0.79/месяц и разворачивается в позицию за один
цикл. Хедж трогать не нужно: цель уже 0, netΔ +0.170 в полосе 0.25,
0 сделок за сутки. Автоматика сама доберёт шорт при SOL ≈ 79.0 и срежет
при SOL ≈ 64.4 — ожидаемое поведение, не тревога.

**6. Найдено: BUG-025 (средний приоритет, живой хедж, фикс НЕ применён).**
Дельта перпа в `jupiterPerpsEngine.ts:392-393` (и дубль на 824-825)
считается как `notional / СПОТ`, тогда как выплата позиции (и её же
формула uPnL строкой выше, :352) определяется ценой ВХОДА:
`sizeUsd / entry` — величина постоянная всю жизнь позиции. Сегодня:
истинная дельта шорта 173.914845 / 74.951934 = **2.320358 SOL**, движок
показывает 2.281606 → расхождение **0.0388 SOL = 16% полосы**. Ошибка
растёт линейно с удалением от входа: на ±10% это ≈0.21–0.26 SOL, то есть
целая полоса — может и выдумать ненужную сделку, и спрятать нужную.
Эмпирическое подтверждение — та самая проверка п.2: рамка «через вход»
предсказала +0.507 при факте +0.507, рамка «через спот» дала бы +0.555.
Направление ошибки: SOL растёт → перешортим, SOL падает → недошортим.
Фикс на одну строку (вернуть `entryPriceUsd` из `fetchSide()` и делить на
него) + перегенерация общих векторов + `cargo test`. Ждёт одобрения.

---

## 2026-07-26

### Session 37 — срез A23 #5 = −7.48/−8.01: ВТОРОЙ минус подряд → правило сработало, возврат на SOL/USDC поддержан данными; мостовая качель поймана вживую ×2 за день ($552 оборота) + пробой notional-потолка объяснён; найден BUG-024 (vitals_open сторожа не чистится)

**Задание оператора:** «сделай срез и проверь то чего мы хотим добиться».

- **Срез #5 (2026-07-26T10:56Z, 184.7ч от якоря):** TOTAL $319.59 = LP
  148.88 (позиция 6xQvoaCnB2oEd4vhCeZAvrHB5xdtebwmGvf8atnLFsvw, капитал
  1.983945 SOL) + кошелёк 104.07 (0.684272805 SOL + 50.812475 USDC +
  0.032550142 HYPE) + шорт 66.64 (залог 66.980435, uPnL −0.305098) →
  **−7.48 формула / −8.01 честно** против якоря 327.07. Окно #4→#5
  −6.05 при почти неподвижном SOL (75.44→75.04) — минус сделала пила,
  не рынок.
- **Механика минуса:** 24 июля ещё 2 выхода из диапазона (recenters
  15:01, 21:50; 25–26 июля — ноль) = traversal-IL, и **двойная живая
  качель моста**: increase $125.46 → decrease $133.70 → increase $145.00
  → decrease $147.89 ($552 оборота перпа впустую; первый increase выпил
  ровно всю USDC кошелька: 41.40/0.33 = $125.45). Темп комиссий позиции
  ~0.18%/день — вдвое ниже штилевых 0.4–0.5%.
- **VITALS «notional above auto-cap» (24 Jul 23:51Z) разобран:** потолок
  честно соблюдался при каждом increase (кошелёк толстый, LP закрыта),
  а при переоткрытии LP вход авто-потолка схлопнулся 405→217 при живом
  gross 321.81 — пробой = подпись мостового двойного счёта, не сломанный
  enforcement. Сам рассосался декризом уже в полёте (00:05Z OK).
- **Выверка (обязательный блок):** tx-audit 24 Jul 10:34Z→now: 32 tx,
  все классифицированы, 3 внешних = пыль ≤1 лампорта; сеть 0.000815133
  SOL/13 наших tx; liq 1.380× ✓; netΔ −1.932 vs −1.97 в коридоре ✓;
  контейнеры без рестартов; hype 48ч: 1 ошибка = известная BUG-017,
  самовосстановилась. **BUG-024 заведён:** watchdog.state держит
  vitals_open с 23 июля несмотря на фактическое восстановление —
  переменная никогда не сбрасывается.
- **Вердикт по правилу среза #3 (два минусовых среза подряд):**
  сворачивать A23, возвращаться на SOL/USDC (bins 28; перед входом —
  замер плотности комиссий пула кампании vs старого пула). Честный итог
  −8.01 к тому же ниже стопа −5, предложенного на срезе #4. Живое
  сворачивание — ждёт «да» оператора (план в итоговом сообщении сессии).

**СВОРАЧИВАНИЕ ВЫПОЛНЕНО (той же сессией, ~11:29–11:45Z, по «Да,
сворачивай по плану»):** оба контейнера остановлены чисто; HYPE-LP
6xQvoaCnB2oEd4vhCeZAvrHB5xdtebwmGvf8atnLFsvw закрыта
(EtQf1qkmPR5hoFa45DkbnuwDLrbCyJbghcQguK9Nwq2Gb3QQiobA3Nx6qaWSFk6UXG42bEyJsQyzfgXJcxFEbWb),
весь HYPE 0.93889276 продан → 0.735891959 SOL по средней без
проскальзывания
(5qrJTD2nxLba55qa8h9JrA8nVwQ7t5o9HUrNgRvKaLaUQQTuVNb6Va417KxQ4rVa937bJ8HyAwB9Va2F1afQEhE4);
мост HEDGE_TARGET_DELTA_SOL=0 (сервер+локально), контейнер пересоздан
(грабли повторены: compose start НЕ перечитывает env — только recreate);
парковка дельта-нейтральна: 2.750958478 SOL + 50.812475 USDC + шорт
2.3163 SOL, контроллер «in band» netΔ +0.128 vs 0; hype-сервис убран за
compose-профиль ["hype"] — голый up -d его больше не поднимет.
**Итог A23 финально: TOTAL при закрытии ≈ 323.96 → −3.11 формула /
−3.64 честно за 7.7 дня** (лучше среза #5 на возврат ренты позиции
≈ +4.28 — формула среза ренту не считала, класс BUG-022).
**ВХОД ОТЛОЖЕН — сработал замер из плана:** пул кампании фактически
умер (оборот за час $135, за сутки $1.05M, плотность 0.0364%/день),
старый жив ($5.34M/сутки, 0.0449%/день), но его перевес 1.23× меньше
2.5× разницы ставок (правило A24 — для наших проходов кампания всё
равно лучше), и оба в ~5 раз ниже уровней 20 июля, ради которых
затевался возврат. Симулятор на свежих окнах (до 26 Jul 10:00Z):
старый пул на шаге 4 месит фарш при любой ширине (28/40/56/70 корзин →
edge −0.9(93% вне пула)/−20/−22/−25 за месяц), кампания bins28 месяц
−13.5 / неделя +0.32 ≈ ноль. Решение: стоим в дельта-нейтральной
парковке (цена — carry ≈ $0.80/мес), критерии входа записаны в BACKLOG
A23 (плотность ≥0.15%/день + недельный edge ≥ +2).
NOTE для будущих tx-audit: закрытие LP и продажа HYPE выполнялись С
ЛОКАЛЬНОЙ машины → их db-строки лежат в ЛОКАЛЬНОМ data/pnl.db; аудит
против СЕРВЕРНОЙ базы покажет эти 2 транзакции (11:32Z meteora LP,
11:34Z jupiter swap) как наши без db:-тега — это ОЖИДАЕМО, не тревога.
Пост-миграционная проверка (11:53Z): main Up, 66 циклов/16мин, хедж 0
сделок с миграции, netΔ +0.128 in band, шорт нетронут до цента
($173.914845/$66.980435), liq 1.383×, VITALS 0 (только boot-баннер),
сторож отработал окно обслуживания штатно (BAD 11:35 → OK 11:45),
hype-контейнер лежит и не поднимается голым up -d.

## 2026-07-24

### Session 36 — операторская тревога «куда делись деньги» → кражи НЕТ (аудит 62 tx чист); реальный инцидент: хедж был отключён 29.7ч (RPC 503 ×5); рестарт по «да» оператора; срез A23 #4 = −1.44/−1.97 — первый минус

**Что случилось (нарратив инцидента):**
- Оператор не увидел LP-позиций и «приличной суммы» в кошельке. Разбор по
  runbook (triage → tx-audit → балансы): LP-позицию hype-экземпляр закрыл
  сам в 08:37Z по выходу цены из диапазона (ADR-026, 5-й цикл
  закрыть/открыть с 21 июля) и ждал выдержку на вход; «пропавшие» $70.91 —
  залог внутри перпа Jupiter (в кошельке не виден); остальное — падение
  рынка (SOL $81.5→$75.4 за 3 дня). tx-audit 2026-07-21T08:00Z→now: 62
  транзакции, все наши/киперские; 1 внешняя — нулевая (ΔSOL 0, ΔUSDC 0,
  комиссия не наша).
- РЕАЛЬНЫЙ инцидент: 2026-07-23T05:04:40Z хедж главного экземпляра
  отключил предохранитель «5 ошибок подряд» (шторм RPC 503 у Helius,
  подтверждено data/logs/bot.log). Шорт заморожен 29.7ч на 2.4502 SOL —
  по счастью это держало истинную дельту портфеля ≈ +0.31 SOL (проектный
  остаток) даже при закрытой LP. В тот же шторм сам перезапустился
  hype-контейнер (сторож его НЕ мониторит — известная дыра). VITALS у
  сторожа висели открытыми с 23 июля.
- Действие (по «да» оператора): дождался переоткрытия LP
  (8PgptLe3TFba5M1WrWqv9qbNxJDCfhpDXmtStoa9xw96, 50.65/49.35, диапазон
  0.7545–0.7947, 10:44Z) — рестарт ДО переоткрытия задвоил бы счёт через
  мост −1.97 — затем `docker compose restart delta-neutral-bot` 10:45Z.
- Проверка: грация 20 циклов «нет LP» отработала, дальше стабильно
  «Hedge: no action, in band», netΔ −2.044…−2.048 vs цель −1.97, band
  0.25; свежих VITALS/ошибок 0 (единственная красная строка — штатный
  баннер «HEDGE IS LIVE»); ликвидация 1.418× при споте 75.44. Сторож
  снимет vitals_open на ближайших прогонах — если пометка не уйдёт, это
  отдельный дефект сторожа, проверить на следующем срезе.
- Срез #4 записан в BACKLOG A23 (первый минус: −1.44 формула / −1.97
  честно; окно #3→#4 −5.01 = базис HYPE/SOL −3.3% + остаточный лонг на
  падении SOL + издержки 5 перезаходов − комиссии). По правилу среза #3
  вердикт выносит срез #5 (25 июля).
- Эскалация приоритета: «hedge input = hype-pool value» (мост → честная
  проводка) теперь СРОЧНО — качель моста стреляет на КАЖДОЙ выдержке на
  вход в пильном режиме (зафиксирована живая: 22 июля 16:14–21:01,
  increase −40.85 USDC → блоки при $0 USDC → decreases 44.69 + 81.06).

## 2026-07-21

### Session 35 — срез A23 #3 (+3.58/+3.05, лучший из трёх) + head-to-head #3: правило переоткрытия SOL/USDC СРАБОТАЛО; найден BUG-023 (качель от нулевого чтения баланса)

**Задание оператора:** «Сделай срез по пулам. Прогони с ним дата и посмотри
разницу между стратегиями текущей и возможной на USDC sol.»

**Срез A23 #3 (2026-07-21T08:22Z, 62.1ч от якоря):** TOTAL **330.65** vs
327.07 → **+3.58 формула / +3.05 честно** (тренд честного: +1.26 → +0.74
→ +3.05). Состав: пул 157.05 (1.999995 SOL, комиссии окна +0.0098153
SOL=$0.77, темп 0.435%/день) + кошелёк 103.96 (0.802151852 SOL +
39.544888 USDC + 0.022725965 HYPE) + шорт 69.65 (76.384078 − 6.720932 −
0.016845). Окно +2.31 = комиссии +0.77 + остаточный лонг ~0.33 SOL ×
рост SOL 75.98→78.52 ≈ +0.8 + базис HYPE +1.21% ≈ +0.56 − carry 0.03.
Проверка полная: tx-audit 6 транзакций все классифицированы (2 неудачных
keeper TX2 стоили 0), main 5983 цикла/макс разрыв 15с, hype каденс 60с,
0 ресентеров у обеих машин, ликвидация 1.349× (порог 1.3 пробьётся при
SOL≈81.5 — следить), сетевые комиссии окна 0.000010 SOL.

**BUG-023 (новый, открыт):** 2026-07-20T19:10:42Z чтение баланса кошелька
вернуло 0 на один цикл (VITALS поймал и через 9с отпустил), но контроллер
хеджа съел ноль как факт → живой decrease_short $33.57, через кулдаун
обратный increase_short $38.65. Ущерб ≈ 0 (переоткрытие на 0.13 НИЖЕ
закрытия), но в шторм такая качель дорогая; decrease не блокируется
оградами по построению. Фикс предложен (пропуск цикла хеджа при
walletSol=0 / подтверждение 2 циклами) — ждёт одобрения оператора.

**Head-to-head #3 (мастер HYPE продлён до Jul 21 08:16Z, k=75.904171
совпал; один календарь, LP 148, abs=USD):** срез-62ч HYPE +2.07 vs
SOL/USDC +1.51 (живой +3.05 подтверждает HYPE); неделя HYPE −7.44 (b28
−6.14) vs SOL/USDC +1.48 (0.49: +1.73); месяц HYPE −13.50 vs SOL/USDC
+1.03 (0.49: +1.75). Вся разница недели/месяца = базис HYPE-к-SOL до
якоря живого теста. **Правило Session 33 сработало** (штиль ≥ месяца +
недельное окно за SOL/USDC): рекомендация — дожать вердикт A23 до
~Jul 24–25; если срезы #4–#5 потеряют плюс, возврат на SOL/USDC теперь
поддержан данными. Band 0.49 ≥ 0.25 снова на всех окнах. Дрейф цели
+0.030 — не трогали. Детали: BACKLOG A23 (срез-лог #3 + head-to-head #3),
bugs.md BUG-023.
