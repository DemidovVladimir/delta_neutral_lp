...
### Session 34 (вечер) — Симуляторный марафон: варианты/параметры/хеджи/пулы/пары; скилл simulator переписан под «лучшую возможную итерацию»

**Задание оператора:** погонять стратегию на разных вариантах и параметрах
(хеджи, пулы ликвидности, пары), приоритет — прибыль в долларах; обновить
скилл, чтобы каждая итерация симулятора отражала лучшее известное.

**Инфраструктура (долгосрочный выход сессии):**
- `scripts/pair-candles.ts` — рецепт A20 (GeckoTerminal 1m OHLCV → кэш
  симулятора) стал постоянным скриптом: пагинация c 429-бэкоффом, плоская
  заливка пропусков, режимы `--normalize` / `--scale` / `--splice`.
- Симулятор: новый флаг `--symbol` + защита в `data.rs` — свечи не-SOL/USDC
  пар живут под своими именами (`HYPESOL_1m_*` и т.п.), отсутствие кэша =
  громкая ошибка вместо тихой подмены Binance-данными. Класс «отравленного
  кэша» (Sessions 32–33) закрыт навсегда; неоднозначные GT-файлы удалены
  (восстановимы по рецепту); все тесты зелёные, `--symbol HYPESOL`
  воспроизводит старые числа до цента.
- Мастер HYPE/SOL продлён до Jul 20 20:00Z
  (`HYPESOL_1m_1781697600000_1784577600000.csv`; splice k=75.904171 совпал
  с записанным). Валидация против среза #2 (37ч): 0 ребалансов точно,
  комиссии 0.90 vs 1.00 реальных (−10%), abs +1.08 vs честно +0.74
  (разница = carry 0.03 + разовая усадка коллатерала 0.11 + шум оракула).

**Сетка A — живой фрейм HYPE (месяц 720ч / неделя 168ч / срез 49ч / весь
мастер 800ч):** база (bins 20) edge −1.97 / −2.28 / +1.06 / −0.94;
**bins 28 лучше базы на каждом многонедельном окне — ТРЕТЬЕ независимое
подтверждение** (−1.86 / −2.12 / +0.76 / +0.99; цена — минус 20–30%
комиссий в тихие дни); b28-c5 и bins 32 местами ярче (b32 месяц −0.84),
но не устойчивы на всех окнах — не гонимся за оверфитом; reentry 240
опять месяц-чемпион (+5.12) / неделя-провал — ловушка режима; tol 0.10
паркует машину 61% времени; штормов в окне 0. **Долларовый вывод: никакой
параметр не меняет знак — настройка двигает ±1–2 USD/мес, качель базиса
HYPE-vs-SOL ±20.**

**Сетка B — SOL/USDC на свежих реальных свечах (те же календарные окна):**
хеджированная машина (band 0.25) в долларах: месяц +0.54 / неделя +1.94 /
336ч −2.14 / срез-49ч +0.94 — около нуля, знак задаёт окно; band
0.49≈0.62 ≥ 0.25 везде (перп-сделок −25%, месяц +1.71) — «шире полосу»
подтверждено в очередной раз; unhedged band 99 = чистая лотерея
направления (+22.78 на ралли — не edge). Режим SOL/USDC по-прежнему ~4×
тише исторического (31 ребаланс/мес, 0 штормов) — правило Session 33
(пере-открывать вопрос только недельным окном) продолжает тикать. Жирный
тир (step 20) в симе выигрывает (+9..11/мес), но на цепи мёртв (A18) —
артефакт модели комиссий, не предложение.

**Сетка C — опрос пар (одно окно 336ч Jul 6→20, чистая механика band 0.49
/ живой abs band 99):** референсы SOLUSDC +4.63/+1.04, HYPE +4.21/−8.47.
PUMP/SOL −28.83 — свежие данные ОТВЕРГАЮТ июльское «пограничен»;
Jimothy/SOL (step 50 / 0.5%) — перманентный шторм (745 пауз) + лотерея
мешка (+452 на пампе +263%) = класс ANSEM + систематическая ошибка
выжившего; MET/SOL −8.0/−16.5 и USELESS/SOL …/−41.2 — отказ; JUP/SOL
(step 80 / 0.15%) +6.34/−10.73 — единственная механически положительная и
спокойная альтернатива (6 ребалансов, 4.8% вне пула), но базис JUP так же
нехеджируем (−14%/2нед) — точка данных, не предложение. Пулы:
альтернативный HYPE/SOL `6oQ9wVex4mKZti2GsGCfD8FWTMMC9PLQkztRU5cd6MK8`
(step 4 / 0.04%) — плотность комиссий пула ×3 нашей, но механика −5.14 vs
+4.21 (паттерн A24) — отказ без живого замера tx/h; плотность старого
SOL/USDC пула снова выше кампанийного (0.34 vs 0.21 %/день) — при любом
возврате в SOL/USDC перемерить venue.

**Скилл `.claude/skills/simulator/SKILL.md` переписан:** канонические
фреймы verbatim-флагами (живой A23, кампанийный, чистая механика, экран
пары), правило метрики (EDGE — сравнение конфигов на одном пути; ABSOLUTE
Δequity — долларовый вердикт), реестр мастеров + команда продления,
гигиена кэша, результаты Session 34. Проверка `rg '\$[0-9]'` чиста.

**Вердикт для оператора:** на масштабе ~150 USD обе конструкции ≈ 0 ±
режим; единственное предложение с данными — bins 20→28 на hype-экземпляре;
нового кандидата лучше «HYPE/SOL + шорт» опрос не нашёл.

**ПРИМЕНЕНО (приказ оператора, ~20:39Z):** `.env.hype`
`AUTO_TUNE_BIN_COUNT` 20→28 (локально и на сервере, штамп STRATEGY_VERSION
сохранён), hype-контейнер пересоздан. Проверка: в контейнере переменная 28,
конфиг цикла `binCount: 28`, позиция та же
`2BKShoLZiPUmmg7fepp6hQn3R7kTnZRnT3RfUAiWd7tg` найдена на цепи, первый цикл
чистый («Position balanced — no action needed», 37.3/62.7), оба контейнера
Up. Живая позиция остаётся 21-корзинной до следующего естественного
пересоздания — 28 корзин применятся на нём; допуск реентри (доля ширины)
и полоса масштабируются сами.

### Session 33 — Срез A23 #2: +1.28 формула / +0.74 честно (остаёмся в плюсе; окно #1→#2 −0.52 — качель базиса HYPE-vs-SOL)

**Срез A23 #2 (07:30Z, 39.2 ч от якоря 2026-07-18T18:16:50Z; 13.0 ч от
среза #1):** TOTAL **$328.34 vs baseline $327.07 → +1.28 (+0.39%)** по
формуле A23; честно **+0.74** (минус выпущенный на якоре uPnL +0.5346).
Слагаемые (pyth 75.97898752): пул 1.978813 SOL = $150.35 (та же позиция
`2BKShoLZiPUmmg7fepp6hQn3R7kTnZRnT3RfUAiWd7tg`, 1.611237923 HYPE +
0.690258857 SOL + несобранные комиссии 0.008550453 HYPE + 0.006415579
SOL; комиссии всего окна 0.0131837 SOL = $1.00); кошелёк БЕЗ ИЗМЕНЕНИЙ
(0.802161852 SOL + 41.304678 USDC + 0.022725965 HYPE) = $103.62; шорт
$74.38 (= коллатерал 75.689564 + uPnL −1.270132 − borrow 0.042562).

Окно #1→#2 = **−0.52**: комиссии +$0.41 не покрыли качель базиса — HYPE
к SOL за окно −1.2% (от якоря −0.4%; удача +0.60 среза #1 наполовину
откатилась). IL от якоря −$0.03 (позиция в диапазоне всё окно, 63.6/36.4).
Темп комиссий: окно #1→#2 **0.499%/день**, полное окно 0.408%/день —
выше нормы (red flag < 0.1%). Разложение честного +0.74: комиссии +1.00,
базис −0.30, IL −0.03, остаточный лонг +0.23, carry −0.04, разовая усадка
коллатерала −0.11.

**Верификация (обязательный блок):** tx-audit `--since 2026-07-19T18:33Z`
= **0 транзакций** (окно стерильно, комиссий сети 0); hedge_actions новых
0 (всего 2 = сетап A23); rebalances hype-экземпляра 0 (mint не менялся);
оба контейнера Up 35h, restarts 0; VITALS 0 / [error] 0 / storm 0 в обоих
контурах с момента среза #1; netΔ −1.9512 vs цель −1.97 «in band» в логе
сервера; liq $105.878722 = 1.394× спота (порог 1.3); vitals_open пуст.
Дрейф цели: пул ex-fees 1.965629 SOL vs −1.97 → −0.004, обновление
HEDGE_TARGET_DELTA_SOL НЕ требуется.

**Найдено (косметика, не чинил):** локальный `pnpm dashboard --json`
печатает `outOfBand: true` — `dashboardData.ts:221` считает
`|netΔ| > band`, игнорируя `HEDGE_TARGET_DELTA_SOL`; при мостовой цели
−1.97 табло всегда «OUT-OF-BAND», хотя контроллер в полосе. Однострочный
фикс (вычесть цель) — при следующем деплое.

Анализатор (standing order, инлайн — окно 13 ч стерильно, сим-сетка
Session 32 свежа): параметры оставить; **оператор решил: bins остаются 20**
(28 НЕ применялось, конфиг не трогали); инженерные фиксы прежние: проводка
«вход хеджа = стоимость hype-пула» (target → 0), watchdog на
hype-контейнер, A14 ping URL.

**Очная ставка с закрытой кампанией на том же окне (запрос оператора):**
машина SOL/USDC переиграна на реальных свечах Binance, тот же календарь
(39 ч, 2319 свечей, продакшн-параметры, fee 6.5 bps по D2). Абсолютное
Δequity: кампания **как была** (LP 91) **+0.67** против живых **+0.74** —
ничья (разница 0.07 много меньше модельной погрешности ±10%); кампания
**на тех же деньгах в LP** (148) **+1.70** — на ЭТОМ окне выигрывает.
Механизм: SOL/USDC платит комиссии ~1.75× быстрее на доллар (0.715%/день
сим против 0.408%/день реальных на HYPE/SOL), но отдаёт это обратно
пересечениями — 30% времени вне пула, 2 пересоздания, churn перпа $240,
тогда как HYPE/SOL просидел в диапазоне всё окно с нулём пересозданий.
⚠ Окно ЛЬСТИТ кампании: 1.2 пересоздания/день против исторических ~4.5
(срез #1: 13 пересечений за 2.89 д) — вчетверо спокойнее её обычного
режима. Месячная очная ставка не изменилась: кампания −6.48/мес против
−0.85/мес. **Вердикт прежний — не возвращаемся**; если затишье по SOL
продержится неделю+, перезадать вопрос на недельном окне. Детали:
BACKLOG A23.

**Raydium SOL/USDC проверен (запрос оператора «прогони на симуляторе») —
ОТКЛОНЁН (BACKLOG A24).** Плотность комиссий на доллар TVL (оборот ×
тариф, мерило venue-нейтральное): Meteora SOL/USDC 0.2025%/день
(2.02× оборот × 0.1%) > Raydium CLMM 0.1388%/день (3.47× × 0.04%) >
Meteora HYPE/SOL 0.0962%/день (0.48× × 0.2%). То есть лучший пул Raydium
платит на **31% МЕНЬШЕ** того пула Meteora, который мы уже закрыли — его
в 1.7 раза больший оборот не перекрывает в 2.5 раза более тонкий тариф;
сходится с собственным feeApr Raydium (50.62%/год день). Сим на том же
39-часовом окне: тариф Meteora 6.5 bps → +1.70, тариф Raydium 4 bps →
+1.33. **Методическая оговорка (важная, многоразовая): симулятор НЕ
различает площадки** — его модель комиссий считает только наши
собственные пересечения диапазона и не знает про объём пула, поэтому
«прогнать Raydium» сводится к «прогнать нашу машину на 4 bps»; вопросы
площадки решаются измеренной плотностью комиссий (правило A21). Плюс
блокеры: другая программа `CAMMCzo5YL8w4VFF8KVHrK22GGUsp5VTaW7grrKgrWqK`
(нужен новый адаптер площадки, у бота только Meteora DLMM) и пара та же
SOL-против-стейбла — смена площадки не лечит первопричину.

---

## 2026-07-19

### Session 32 — Срез A23 #1: +1.79 (первое положительное окно); сетка симулятора на свежих свечах — bins 28 единственный устойчивый кандидат

**Срез A23 #1 (18:33Z, 24.3 ч от якоря 2026-07-18T18:16:50Z):**
TOTAL **$328.86 vs baseline $327.07 → +1.79 (+0.55%)** по формуле A23;
честно ≈ **+1.26** (на якоре из TOTAL был выпущен uPnL шорта +0.53 — вход
по blended 75.465 при споте 75.249; вычитается при сравнении; шкала
±0.2 — движок ценит шорт по oracle 75.55, портфель по pyth 75.62).
Слагаемые: пул 1.9846 SOL ($150.06, из них комиссии окна 0.007849 SOL =
$0.59), кошелёк БЕЗ ИЗМЕНЕНИЙ (0.802161852 SOL + 41.304678 USDC +
0.022725965 HYPE), шорт-эквити $75.46 (= коллатерал 75.689564 + uPnL
−0.201 − наросший borrow 0.029). Разложение: комиссии +0.59, IL −0.09,
базис HYPE-vs-SOL +0.60 (HYPE +0.8% к SOL — удача, не структура),
остаточный лонг +0.11, carry −0.03, усадка коллатерала −0.11 (разовая —
open-fee добора, базлайн снял цифру до списания). Верификация: tx-audit
11 tx все классифицированы, **0 транзакций после снимка базлайна** (24 ч
тишины: LP в диапазоне, хедж в полосе −1.9666/−1.97); hedge_actions = 2
(сетап A23); rebalances hype-экземпляра = 0; VITALS 0; restarts 0; liq
$105.88 = 1.40× спота. Темп комиссий 0.393%/день (норма, red flag <0.1%;
при 2.6 ч было 0.448%). Дрейф цели: пул 1.9767 SOL ex-fees vs −1.97 →
+0.007, обновление HEDGE_TARGET_DELTA_SOL НЕ требуется (пол полосы 0.25).

**Симулятор (по просьбе оператора — «позитивные варианты»):** кэш свечей
HYPE/SOL продлён GT-выгрузкой до Jul 19 18:00Z (склейка k=75.904170,
разброс 6-й знак; мастер `SOLUSDC_1m_1781697600000_1784484000000.csv`).
Аутентичность на живом окне A23: сим 0 пересозданий = реальность 0;
комиссии сим $0.51 vs реал $0.59 (+16% реал — пессимизм 12.5 bps
подтверждён). **Уточнение рамок: band 0.49 = фиктивный HYPE-перп (A19 —
не существует), живой A23-конструкции соответствует band 99, и живой
срез ≈ АБСОЛЮТНОЕ Δequity этой рамки (SOL-шорт лишь конвертирует
SOL-метрику в USD), а не EDGE-vs-hold.** Сетка (месяц 774h / свежая
неделя 168h / живой день, lp 148): текущие параметры месяц −1.05;
**bins 28: месяц +0.64, неделя тай-или-лучше во всех рамках —
единственный кандидат, устойчивый везде** (механизм: ±2.8% vs ±2%, 37 vs
47 пересозданий, вне пула 16% vs 26%; цена — −27% комиссий в тихий день:
сим 0.37 vs 0.51). Отвергнуто данными: bins 14 (−5.83) / 40 (−2.09);
confirm 5 (шум) / 30 (−2.49); tol 0.15 (−3.85) / 0.30 (−3.42); **reentry
240/360 (+7.7/+14.3 на месяце) — ЛОВУШКА РЕЖИМА**: весь выигрыш = уход
от пилы падающего месяца, на свежей неделе −5.0/−5.9 (хуже базы −2.98) —
корридор не спасает от медленного сползания (те же пересечения, минус
комиссии ожидания). Ожидающий мешок в симе ДЕРЖИТ HYPE-экспозицию
(netΔ +1.1 вне пула) — парковка не нейтральна.

Рекомендация оператору: параметры оставить (или bins 20→28 — маленький
устойчивый плюс, одна строка .env.hype, решение за оператором);
инженерные фиксы прежние — проводка «вход хеджа = стоимость hype-пула»
(target → 0), watchdog на hype-контейнер, A14 ping URL.
