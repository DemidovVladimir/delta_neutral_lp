...
- `REENTRY_CONFIRM_MS` + `REENTRY_TOL_FRAC` (ADR-026 «выдержка на вход»: recenters CLOSE fast but re-open only after the price holds within ±(tol × range width) of an anchor for this long — the anchor resets on every breakout, so trends/slow saws keep the machine out of the pool, hedged-neutral in the wallet; storms extend the wait; the wait survives restarts and gates auto-create; 0 = off/instant rollback; production 7200000 / 0.15)
